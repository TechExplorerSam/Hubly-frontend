...
const MainChatbot = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:9001/Chatbot/getchatbotstylesforuser`,{
      params:{
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response=  await axios.post('http://localhost:9001/Chatbot/registeranewuserthroughchatbot', formData);
        ...
        const response= await axios.post('http://localhost:9001/Chatbot/loginchatbotuser', {
  ...
  const fetchInitialMessages = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:9001/Chat/getAllChatsForChatbotUser/${currentTicketId}`);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      console.log('Sending message to server:', input);
       console.log('Current Ticket ID:', currentTicketId);
      if (!currentTicketId) {
        const response = await axios.post('http://localhost:9001/Chatbot/createanewchatwithuser', {
    ...
    const response=  await axios.post(`http://localhost:9001/Chatbot/sendmessagetochat/${currentTicketId}`, {
