...
const {ticketName}=useTicket();
   ...
   const handleReplySubmit = async () => {
      ...
      try {
         await axios.post('http://localhost:9001/Chatbot/sendreplytochatbotuser', {
            chatId: chat._id,
   ...
   const classifyChatAsMissedChat = () => {
    ...
    axios.post('http://localhost:9001/Chat/classifychatasmissedchat', {
       userId,
       ticketId,
    })
       .then((response) => {
 ...
 const checkIfChatAssignedToAnotherUser = async () => {
   ...
   try {
     const response = await axios.get(`http://localhost:9001/Chat/checkifchatassignedtoanotheruser`, {
       params: {
         userId,
         ticketId,
       },
