...
const TeamMembers = () => {
    ...
    if (editIndex !== null && editMemberId) {
      try {
        const res = await axios.put(
          `http://localhost:9001/TeamMember/updateteammemberbyid/${editMemberId}`,
          {
            adminUserId: userId,
      ...
      try {
        const res = await axios.post(
          `http://localhost:9001/TeamMember/createnewteammember`,
          {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const res = await axios.delete(`http://localhost:9001/TeamMember/deleteteammemberbyid/${memberIdToDelete}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
